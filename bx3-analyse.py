...
def parse_pcap_file(language,key,pcap):
	# Open the pcap file
	f = open(pcap, 'rb')
...
def bx3_decode(http,language,key):
	
	# if len(http.body) >=20000:
	# 	print("1")
	# 	exit
	if language == 'java':
		try:
...
if __name__ == '__main__':
	...
	# print(sys.argv)
	if len(sys.argv) ==4:
		print("key="+sys.argv[1]+",pcap="+sys.argv[2]+",language="+sys.argv[3])
		pcap=sys.argv[2]
		language = sys.argv[3]
		key = sys.argv[1]
	# pcap  = 'bx_upload.pcap'
	...
	parse_pcap_file(language,key,pcap)
